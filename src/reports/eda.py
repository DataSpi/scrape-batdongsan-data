...
df_merge = db.fetch_to_dataframe(conn=db_conn, query="select * from re_silver.real_estate")
prj = db.fetch_to_dataframe(conn=db_conn, query='select "projectId", name as project_name from re_bronze.m_projects')

df_merged = df_merge.merge(prj[['projectId', 'project_name']], left_on='projectId', right_on='projectId', how='left')
